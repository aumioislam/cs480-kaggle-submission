...
from PlantModels import ViTModel
...
if __name__ == "__main__":
    ...
    model = ViTModel(loss_fn=nn.MSELoss(), scheduler=None, lr=0.001, freeze=True)
    trainer.fit(model, train_loader, val_loader) # type: ignore

    # generate submission file
    best = ViTModel.load_from_checkpoint(trainer.checkpoint_callback.best_model_path) # type: ignore

    val_result = trainer.validate(best, dataloaders=val_loader, verbose=False)
    print(f"val_r2: {val_result[0]['val_r2']}")
    submission_name = f"lmbda-vit-b-16-submission-v{trainer.logger.version}-r2-{val_result[0]['val_r2']:.4f}" # type: ignore
